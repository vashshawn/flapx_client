...
class LiteDogeClient
  class Client
    ...
    def http_post_request(post_body)
      ...
      raise LiteDogeClient::HTTPError.new(response)
    end
    ...
    def get_response_data(http_ok_response)
      ...
      raise LiteDogeClient::RPCError.new(resp['error']['message']) if resp['error'] and http_ok_response.class == Net::HTTPInternalServerError
      resp['result']
    end
    ...
    def get_defaults
      LiteDogeClient.configuration.instance_variables.each.inject({}) {|hash, var|
